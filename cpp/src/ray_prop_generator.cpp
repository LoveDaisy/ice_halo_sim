...
public:
    ...
    Input<Buffer<int>>      faceId{"faceId", 1};
    ...
    void generate() {
        ...
        Expr t = select(abs(c1) > EPS, (a1 - b1) / c1, -1);
        ...
        Expr alpha = select(abs(c1) > EPS, (a2 + b2) / c1, -1);
        ...
        Expr beta = select(abs(c1) > EPS, -(a3 + b3) / c1, -1);

        Func t_all{"t_all"};
        t_all(y, yf) = select(alpha >= 0 && beta >= 0 && alpha + beta <= 1 
            && t > EPS && yf != faceId(y), t, MAX_FLOAT);
    ...
    void schedule() {
        if (auto_schedule) {
            ...
            faceId.dim(0).set_bounds_estimate(0, DEFAULT_NUM);
