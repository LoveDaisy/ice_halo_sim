...
- **Core / rendering architecture**: `accumulator-consumer-architecture.md`, `raypath-rayseg-architecture.md`, `raypath-symmetry.md`, `coordinate-convention.md`, `crystal-orientation-sampling.md`, `ev-pipeline-architecture.md`, `adaptive-brightness.md`, `filter-architecture.md`
  - `ev-pipeline-architecture.md` §2.6/§4/§7 + `adaptive-brightness.md`/`.zh.md` §3 — **两种曝光锚点（`ev_mode`: relative 默认 / absolute）的机制与契约**：`relative` 自锚到当前帧自身 P99（画面外观随 `ray_num` 增长不变，但 config 本身不决定输出亮度）；`absolute` 锚到光源发射能量（`SimData::emitted_energy_` → `LUMICE_RawXyzResult::emitted_energy`），使不同 config 在同一 EV 下直接可比。⭐**两条已被裁定接受的代价，写成契约而非缺陷**：① `relative` 下 config 不完整决定亮度是"保外观"的必然对偶；② 欠采样场景在任何固定 EV 下都会随累积变暗，这与选哪个分母无关（两种分母的 N 标度斜率相同），`relative` 靠 N 相关的锚增益把它抵消掉，`absolute` 没有抵消项而使其显形——是诚实的 MC 行为，不是"跑越久越坏"。⭐**迁移律**：`new_scale / old_scale ≡ landed_fraction`（落帧能量/发射能量），可由 C API 两个字段自行算出；实测位移全天球无 filter −0.038…−0.002 stop、窄视场无 filter −0.67…−1.25 stop、filter/高 `ms_prob` −0.97…−10.47 stop。⭐**诚实边界**：可比性只覆盖同 lens/FOV/分辨率，跨 lens 差一个 `landed_fraction` 比值，故意不做逐像素 `Ω_p` 归一（会重新加权改变画面外观）。⚠️**别把这条读成「`Ω_p` 处处被否决了」**：它否的是**在 core 侧把 `Ω_p` 除掉**；GUI 预览侧的毛病方向相反（重采样等面积纹理 ⇒ 一点 `Ω_p` 都没有），已由 §7.5 补上**相对照度**（`Ω_p` 归一到轴上值，`src/gui/preview_jacobian.hpp` + shader `relIllum*`）——两条合起来说的是同一件事：两侧都该是相机语义。§7.5 同时记着为何归一化取轴上值而非 `Ω_texel_source`（两侧各自自锚，`Ω_p` 的**量级**在 CLI 侧已被自己的 p99 消掉，缺的只有**形状**；用绝对比值会引入一个由画布角分辨率决定的全局增益，实测 0.331×/16×，把已绿的场景打红），以及由此决定的 `absolute` 行为（轴上亮度不随 fov 变）。⭐**§2.7 测光规则（`visible` 不参与测光）**：`kRelative` 的 P99 取**整张缓冲**（全天），`visible` 的半球裁剪发生在测光**之后**——它决定你看到什么，从不决定尺子量了什么；这是与 GUI 共用的**同一条**规则（GUI 的 `ComputeEvAuto` 锚的就是全天纹理，因为 `kSimCommit` 把 `visible` 钉成 FULL），按掩码过滤 P99 等于在专门消灭 core↔GUI 分歧的地方新开一条分歧。⭐代价已写成契约：`visible` 统一为显示层裁剪后，被排除半球首次进入 P99 样本 ⇒ **既有 CLI + 单镜头族 + `visible != full` + `relative` 场景的亮度会移动**（只有这一个组合会；另三族本来就计全天、`absolute` 天然免疫、GUI 一个像素不动）。⚠️**方向不是单向的**：排除区是暗天空则 P99 下移⇒变亮；排除区装着主要光晕（高仰角 + `lower`）则 P99 上移⇒**变暗**，且最坏点在这一支（实测 −1.68 stop = 3.2× 变暗，另一端 +0.15 stop）。⛔ 与 §7.2 的 `landed_fraction` 不同，这个位移**没有闭式**（取决于能量在裁剪边界两侧怎么分），不得外推；`intensity_factor` 可精确抵消，`ev_mode: absolute` 不受影响。改曝光归一化 / `ev_mode` / `ExposureScale()` / `emitted_energy` / **测光是否看 `visible`** / **GUI 预览的投影亮度**前先读。
