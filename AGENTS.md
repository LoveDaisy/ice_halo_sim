...
- **Core / rendering architecture**: `accumulator-consumer-architecture.md`, `raypath-rayseg-architecture.md`, `raypath-symmetry.md`, `coordinate-convention.md`, `crystal-orientation-sampling.md`, `ev-pipeline-architecture.md`, `adaptive-brightness.md`, `filter-architecture.md`
  ...
  - `raypath-analysis-panel.md` — **光路分析面板设计蓝图**（2026-09-11 owner × assistant 讨论收敛，尚未实现）：
    ...
    四条 owner 裁决：v1 只走 CPU 路且必须显式声明（GPU device-fused 不上 host per-ray 记录）/
    完整链（跨 MS 层）= per-ray 链 id interning 前向携带，与 `RayBuffer::components_` 同一套传播点，
    漏搬一个 swap/fan-out/续传点即静默错位 / ROI = 方向空间锥形 + 分环 + 角半径滑杆 display-time 生效 /
    总能量降序必有，密度排序（能量/立体角）v1 不做。顺带闭环：列表行 →「排除此光路」→ 生成 filter → 重跑，
    v1 只对单段链可用。含诚实边界（每条标注 owner 裁决 or assistant 推断，含一条待核实推断：legacy CPU 路
    或许不必切到 `CpuTraceBackend`）与开放设计点清单（供子任务 plan 裁定）。改光路分析面板 / 链 id 携带 /
    直方图 consumer / 分析运行 C API / 分析面板 GUI 前先读。
