...
TEST(FieldEditorChain, EveryRegisteredFieldHasACoherentDomain) {
...
// Every registered key must also have an editor to draw it. The two lookups are separate functions
// on purpose — one is total and answers "does this leaf happen to have an editor", the other aborts
// on a miss because its caller ships a control for that exact field — but a key that appears in the
// enumeration and has no editor is a row the panel will list and be unable to render.
TEST(FieldEditorChain, EveryEnumeratedKeyResolvesToAnEditor) {
...
// The nullptr answer is designed, not a lookup failure, so it has to actually happen for something.
// If FindFieldEditor never returned nullptr the panel's "read-only here" path would be dead code
// that no longer describes anything.
TEST(FieldEditorChain, AnUnregisteredKeyResolvesToNullptrRatherThanAborting) {
...
TEST(FieldEditorChain, EveryShapeScalarSlotReachesADistinctField) {
...
TEST(FieldEditorChain, TheSyncGroupArrayRoundTripsThroughItsOwnSlotMapping) {
  CrystalConfig crystal;
  ...
  for (int slot = 0; slot < LUMICE_SHAPE_SCALAR_COUNT; ++slot) {
    EXPECT_EQ(read_back[slot], written[slot])
        << "slot " << slot << ": the two directions of the mapping disagree, so the preview and the "
        << "committed scene would draw different crystals";
  }
}

// The face-distance slots are six contiguous entries at the end of the index space, and the header
// asserts that statically. Asserting it dynamically as well is what catches a mapping that compiles
// (the static_assert is about the constants) but routes an index to the wrong array element.
TEST(FieldEditorChain, FaceDistanceSlotsMapToTheirOwnArrayElement) {
  CrystalConfig crystal;
  for (int i = 0; i < 6; ++i) {
...
TEST(FieldEditorChain, EveryShapeScalarSlotHasAUsableDomain) {
  for (int slot = 0; slot < LUMICE_SHAPE_SCALAR_COUNT; ++slot) {
...
// The domain and the scale that traverses it are one answer, not two, and they can contradict each
// other: a logarithmic slider cannot reach zero, so a slot whose minimum IS zero and whose scale is
// pure log offers a bound the control can never produce. prism_h is the concrete case — it must
// allow exactly 0 (a degenerate zero-height prism section) AND stay finely controllable near 0 over
// a wide range, which is why it is the one slot on the log-linear scale rather than the log one.
TEST(FieldEditorChain, ASlotWhoseMinimumIsZeroDoesNotUseAScaleThatCannotReachZero) {
  for (int slot = 0; slot < LUMICE_SHAPE_SCALAR_COUNT; ++slot) {
    const ShapeScalarDomain& domain = ShapeScalarDomainFor(slot);
...
TEST(FieldEditorChain, TheSixFaceDistanceSlotsShareOneDomain) {
