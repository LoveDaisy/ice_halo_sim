...
Motivation
----------
PR#199 flipped the default composite mode from ``dominant`` to ``painter`` at
`src/config/raypath_color_config.hpp::kDefaultCompositeMode`. During that change
...
hard zero rather than an epsilon: the accumulation-symmetry proof above makes
an epsilon unnecessary -- a nonzero B channel would be a genuine tie-break /
accumulation regression worth failing on, not float noise.
...
------------
  ...
  * We do NOT reuse ``test.e2e.image_utils.classify_pixels_by_color_direction``
    -- its docstring still describes the older single-winner painter semantics
    (one winning class per pixel) which is stale under alpha-over blend.
    The tiny local helper below is scoped to this file; if a second consumer
    appears later, lift it to a shared helper then.
...
class TestPainterDefaultComposite(LumiceTestCase):
    ...
    def setUp(self):
        ...
        # guard, so a missing fixture must surface as FAIL, never a silent SKIP.
        self.assertTrue(
            CONFIG.exists(),
            f"fixture {CONFIG} missing -- painter-default regression guard "
            f"must FAIL (not silently skip) if the fixture is deleted/moved",
        )
