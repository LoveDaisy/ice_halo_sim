...
namespace pixel_diff_detail {

// The one flood fill. Both public entry points below reduce their input to a 0/1 mask and hand
// it here, so "largest 8-connected blob" has exactly one definition in the tree. `mask` doubles as
// the visited set and is consumed (cleared on visit) — callers pass a copy they can spare.
inline int LargestComponent(std::vector<unsigned char>& mask, int w, int h) {
  int max_cc = 0;
...
inline PixelDiffResult ComputePixelDiff(const unsigned char* a, const unsigned char* b, int w, int h, int channels,
  ...
  r.max_cc = pixel_diff_detail::LargestComponent(mask, w, h);
  return r;
}

// The same ruler over a difference mask the caller has already built — one byte per pixel, non-zero
// meaning "different". This is the entry the CLI<->GUI parity fixture's lines-only scenes use:
// there the two arms' bytes are not comparable (each composites a half-transparent line in its
// own colour space), so the caller decides per arm which pixels are annotation and hands in the
// XOR of the two memberships. `n_diff` counts the mask's set pixels and `max_cc` is their largest
// 8-connected blob, by the same flood fill as above. `dmax` is 0: a mask carries no per-channel
// delta for it to report. Returns {-1, -1, -1} when the mask does not hold w*h bytes or a dimension
// is not positive.
inline PixelDiffResult ComputePixelDiffFromMask(const std::vector<unsigned char>& mask, int w, int h) {
  if (w <= 0 || h <= 0 || mask.size() != static_cast<size_t>(w) * static_cast<size_t>(h)) {
    return { -1, -1, -1 };
  }
  PixelDiffResult r{ 0, 0, 0 };
  std::vector<unsigned char> scratch(mask.size(), 0);
  ...
  r.max_cc = pixel_diff_detail::LargestComponent(scratch, w, h);
  return r;
}
