// Pure-CPU unit tests for ProjectWorldDirToScreen: analytic expectations checked against the
// preview shader's inverse projection formulas. No GL context and no ImGui — which is why they
// live in gui_unit_test rather than gui_test.
...
TEST(ProjectWorldDir, linear) {
  ...
  EXPECT_TRUE(!IsSentinel(p));
  EXPECT_TRUE(std::abs(p[0]) <= kEps);
  EXPECT_TRUE(std::abs(p[1]) <= kEps);

  const float zenith[3] = { 0.0f, 0.0f, -1.0f };
  auto pz = lumice::gui::ProjectWorldDirToScreen(vp, zenith, 800, 600);
  EXPECT_TRUE(IsSentinel(pz));  // view_dir.z = 0 ⇒ behind-or-on-camera-plane sentinel
}
...
TEST(ProjectWorldDir, fisheye_ea) {
  ...
  EXPECT_TRUE(!IsSentinel(pz));
  EXPECT_TRUE(std::abs(pz[0]) <= kEps);
  EXPECT_TRUE(std::abs(pz[1]) <= kEps);

  const float front[3] = { -1.0f, 0.0f, 0.0f };
  auto pf = lumice::gui::ProjectWorldDirToScreen(vp, front, 800, 800);
  EXPECT_TRUE(!IsSentinel(pf));
  EXPECT_TRUE(std::abs(pf[0]) <= kEps);
  EXPECT_TRUE(std::abs(pf[1] - (-400.0f)) <= kEps);
}
...
TEST(ProjectWorldDir, rectangular) {
  ...
  EXPECT_TRUE(!IsSentinel(pz));
  EXPECT_TRUE(std::abs(pz[0]) <= kEps);
  EXPECT_TRUE(std::abs(pz[1] - (-200.0f)) <= kEps);

  const float nadir[3] = { 0.0f, 0.0f, 1.0f };
  auto pn = lumice::gui::ProjectWorldDirToScreen(vp, nadir, 800, 400);
  EXPECT_TRUE(!IsSentinel(pn));
  EXPECT_TRUE(std::abs(pn[0]) <= kEps);
  EXPECT_TRUE(std::abs(pn[1] - 200.0f) <= kEps);

  const float front[3] = { -1.0f, 0.0f, 0.0f };
  auto pf = lumice::gui::ProjectWorldDirToScreen(vp, front, 800, 400);
  EXPECT_TRUE(!IsSentinel(pf));
  EXPECT_TRUE(std::abs(pf[0]) <= kEps);
  EXPECT_TRUE(std::abs(pf[1]) <= kEps);
}
...
TEST(ProjectWorldDir, dual_fisheye_ea) {
  auto vp = MakeVp(lumice::gui::kLensTypeDualFisheyeEqualArea, 180.0f, 0.0f, 0.0f, 0.0f);
  const float zenith[3] = { 0.0f, 0.0f, -1.0f };
  auto pz = lumice::gui::ProjectWorldDirToScreen(vp, zenith, 800, 400);
  EXPECT_TRUE(!IsSentinel(pz));
  EXPECT_TRUE(std::abs(pz[0] - (-200.0f)) <= kEps);
  EXPECT_TRUE(std::abs(pz[1]) <= kEps);

  const float nadir[3] = { 0.0f, 0.0f, 1.0f };
  auto pn = lumice::gui::ProjectWorldDirToScreen(vp, nadir, 800, 400);
  EXPECT_TRUE(!IsSentinel(pn));
  EXPECT_TRUE(std::abs(pn[0] - 200.0f) <= kEps);
  EXPECT_TRUE(std::abs(pn[1]) <= kEps);
}
...
TEST(ProjectWorldDir, globe_projection) {
  ...
  EXPECT_TRUE(IsSentinel(pn));  // back of sphere — front-hemisphere visibility filter
}
...
TEST(ProjectWorldDir, fisheye_circular_clip) {
  ...
  EXPECT_TRUE(IsSentinel(p_90));  // outside imaging circle (r_norm ≈ 1.33)
  ...
  EXPECT_TRUE(!IsSentinel(p_180));  // inside imaging circle (r_norm ≈ 0.67)
}
