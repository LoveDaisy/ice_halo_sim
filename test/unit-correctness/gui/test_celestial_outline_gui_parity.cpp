...
// =================================================================================================
// Properties of the core mask that do not need the GUI to state
// =================================================================================================

TEST(CelestialOutlineMask, EveryMarkedPixelIsInsideTheVisibleMask) {
  // The annotation is drawn under the same gate as the background (render.cpp PostSnapshot), so a
  // marked pixel outside the render domain would paint a line across a region that images nothing.
  ...
  for (LensParam::LensType t : types) {
    for (RenderConfig::VisibleRange vis : kAllRanges) {
      const RenderConfig cfg = MakeCfg(t, t == LensParam::kGlobe ? 30.0f : 120.0f, 128, 96, vis, /*el=*/0.0f);
      const float short_pix = static_cast<float>(std::min(cfg.resolution_[0], cfg.resolution_[1]));
      const auto rot = lumice::MakeCameraRotation(cfg);
      const auto outline = lumice::BuildCelestialOutlineMask(cfg, rot, short_pix);
      const auto visible = lumice::BuildVisibleMask(cfg, rot, short_pix);
      if (outline.size() != visible.size()) {
        ADD_FAILURE() << "type " << static_cast<int>(t) << ": the two masks of one frame differ in size ("
                      << outline.size() << " vs " << visible.size() << ")";
        continue;
      }
      size_t outside = 0;
      ...
      EXPECT_EQ(outside, 0u) << "type " << static_cast<int>(t) << " visible " << static_cast<int>(vis)
                             << ": horizon marked on " << outside << " pixel(s) the lens does not image";
    }
  }
}

TEST(CelestialOutlineMask, ThePolarViewHasNoHorizonInFrame) {
  // Pointed at the zenith with a narrow FOV, altitude 0 is nowhere in the picture. An empty mask
  // is the correct answer, and a non-empty one would mean the width rule fabricates a line out of
  // whatever gradient it finds.
  const RenderConfig cfg = MakeCfg(LensParam::kLinear, 30.0f, 96, 72, RenderConfig::kFull, /*el=*/90.0f);
  const float short_pix = 72.0f;
  EXPECT_EQ(CountOn(lumice::BuildCelestialOutlineMask(cfg, lumice::MakeCameraRotation(cfg), short_pix)), 0u);
}

TEST(CelestialOutlineMask, DegenerateResolutionReturnsEmpty) {
  RenderConfig cfg = MakeCfg(LensParam::kLinear, 90.0f, 0, 0, RenderConfig::kFull, 0.0f);
  EXPECT_TRUE(lumice::BuildCelestialOutlineMask(cfg, lumice::MakeCameraRotation(cfg), 1.0f).empty());
}
