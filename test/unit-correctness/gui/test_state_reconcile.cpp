// PURE unit tests for the field-tier reconciler geodetics. No GL, no server — which is why they
// live in gui_unit_test rather than gui_test. Peer to test_gui_lifecycle.cpp::reconcile_truth_table
// — the SimState reconciler is the I2 owner; ReconcileGuiEffects is its structural sibling for the
// field-tier layer.
//
// Tests:
...
TEST(GuiStateReconcile, effects_truth_table) {
  ...
  // raypath_color struct-part (combine/match) change → hard-reset lane.
  ...
  // RenderConfigResimEqual compared these fields. This pin prevents a silent re-add.
  {
    ...
    for (const auto& mut : kViewMutators) {
      ...
      EXPECT_TRUE(!e.need_hard_reset);
      EXPECT_TRUE(!e.need_display_push);
      (void)mut.field;  // name is here for debug-print if EXPECT_TRUE fires
    }
  }
  ...
  // Filter presence-toggle at same-shape
  ...
  // be misclassified as hard by the new detector.
  {
    ...
    GuiEffects e = ReconcileGuiEffects(s);
    EXPECT_TRUE(e.need_resim);        // layers diff still fires
    EXPECT_TRUE(!e.need_hard_reset);  // no presence-toggle inside the shape overlap
  }

  // Repush discipline: after InvalidateEffectsBaselines resets the display-push baseline to
  // nullopt, the next reconcile MUST fire need_display_push against a non-empty raypath_color
  // vector. This is what makes z_order take effect immediately after Run: z_order cannot travel
  // through the commit payload, so the display state is re-pushed in full after every DoRun /
  // DoRevert / backend swap.
  {
    GuiState s = MakeBaselineState();
    EXPECT_TRUE(!s.raypath_color.empty());                 // sanity: MakeBaselineState puts one class in
    EXPECT_TRUE(s.last_pushed_display_state.has_value());  // sanity: MakeBaselineState seeds it
    s.InvalidateEffectsBaselines();
    EXPECT_TRUE(!s.last_pushed_display_state.has_value());
    GuiEffects e = ReconcileGuiEffects(s);
    EXPECT_TRUE(e.need_display_push);  // nullopt baseline + non-empty vector → fire
    EXPECT_TRUE(!e.need_resim);        // commit baseline is untouched, no re-sim needed
    EXPECT_TRUE(!e.need_hard_reset);
  }
  ...
  // last_committed_state alongside would flip this to a break.)
  {
    ...
    EXPECT_TRUE(commit_present_before);
    s.InvalidateEffectsBaselines();
    EXPECT_TRUE(s.last_committed_state.has_value());  // preserved across the reset
  }
}
...
TEST(GuiStateReconcile, meta_anti_drift) {
  ...
  //   hard: filters
  //   hard/display split: raypath_color (auto_diff_excluded was dropped when ColorClassConfig
  //     was split into struct and display parts)
  struct Mutator {
    const char* field;
    void (*apply)(GuiState&);
  };
  ...
  for (const auto& mut : kMutators) {
    ...
    EXPECT_EQ(before, GuiEffects{});  // baseline is quiet
    mut.apply(s);
    GuiEffects after = ReconcileGuiEffects(s);
    EXPECT_NE(after, GuiEffects{});
  ...
  EXPECT_TRUE(expected_fields == mutator_fields);
  ...
  EXPECT_EQ(excluded_struct_count, 1);  // use_gpu_backend only
...
TEST(GuiStateReconcile, apply_effects_priority) {
  // effects all-false → GuiState untouched (dirty stays false).
  {
    ...
    EXPECT_TRUE(!s.dirty);
    EXPECT_EQ(s.display_epoch_floor, floor_before);
    EXPECT_EQ(s.p99_raw_y, p99_before);
  }
  ...
  // need_resim only → MarkDirty path (dirty=true; epoch floor untouched).
  {
    ...
    EXPECT_TRUE(s.dirty);
    EXPECT_EQ(s.display_epoch_floor, 0u);  // MarkDirty does NOT raise floor
    EXPECT_EQ(s.p99_raw_y, 1.5f);          // MarkDirty does NOT clear p99
  }
  ...
  // the flatter form.
  {
    ...
    ApplyGuiEffects(s, nullptr, e);
    EXPECT_TRUE(s.dirty);
    EXPECT_EQ(s.display_epoch_floor, 5u);  // raised to committed_epoch
    EXPECT_EQ(s.p99_raw_y, 0.0f);
    EXPECT_EQ(s.snapshot_intensity, 0.0f);
  }
}
