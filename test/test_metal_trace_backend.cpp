...
bool ShouldSkipMetalTests() {
  const char* env = std::getenv("LUMICE_SKIP_METAL_TESTS");
  return env != nullptr && env[0] != '\0' && env[0] != '0';
}

SceneConfig MakeMetalScene(size_t max_hits, size_t ms_layers) {
  SceneConfig scene;
  scene.ray_num_ = 0;
...
RenderConfig MakeRectangularRender() {
  RenderConfig cfg;
  cfg.id_ = 0;
...
double ChannelSum(const std::vector<float>& xyz, int channel) {
  double s = 0.0;
