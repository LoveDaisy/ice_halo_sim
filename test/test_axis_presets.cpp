...
// Verify the Rx(angle) chain term by checking how it maps the unit Y axis.
// In column-major:  Rx(a) * (0,1,0)^T = (0, cos a, sin a).
void ExpectXTiltMapsYUnit(AxisPreset preset, float angle_deg) {
  float r[16] = { 0 };
  DefaultPreviewRotation(preset, r);
  auto v = Mul3(r, 0.0f, 1.0f, 0.0f);
  float rad = angle_deg * kPi / 180.0f;
  EXPECT_NEAR(v[0], 0.0f, 1e-5f);
  EXPECT_NEAR(v[1], std::cos(rad), 1e-5f);
  EXPECT_NEAR(v[2], std::sin(rad), 1e-5f);
}

TEST(DefaultPreviewRotation, ColumnTiltPositiveXAxisMapsYUnit) {
  ExpectXTiltMapsYUnit(AxisPreset::kColumn, 80.0f);
}

TEST(DefaultPreviewRotation, ParrySharesColumnTilt) {
  float r_col[16] = { 0 };
  float r_par[16] = { 0 };
  DefaultPreviewRotation(AxisPreset::kColumn, r_col);
  DefaultPreviewRotation(AxisPreset::kParry, r_par);
  for (int i = 0; i < 16; ++i) {
    EXPECT_FLOAT_EQ(r_col[i], r_par[i]) << "index " << i;
  }
}

TEST(DefaultPreviewRotation, PlateNegativeXTiltMapsYUnit) {
  ExpectXTiltMapsYUnit(AxisPreset::kPlate, -10.0f);
}

TEST(DefaultPreviewRotation, LowitzPositiveXTiltMapsYUnit) {
  ExpectXTiltMapsYUnit(AxisPreset::kLowitz, 60.0f);
}

// Random / Custom share the isometric Ry * Rx view — verify both produce identical matrices
// (one source of truth) and that mapping unit Z lands with non-zero X-projection
// (distinguishing it from the pure-X tilt presets).
TEST(DefaultPreviewRotation, RandomEqualsCustomIsometric) {
  float r_rand[16] = { 0 };
  float r_cust[16] = { 0 };
  DefaultPreviewRotation(AxisPreset::kRandom, r_rand);
  DefaultPreviewRotation(AxisPreset::kCustom, r_cust);
  for (int i = 0; i < 16; ++i) {
    EXPECT_FLOAT_EQ(r_rand[i], r_cust[i]) << "index " << i;
  }
}

TEST(DefaultPreviewRotation, IsometricMapsZUnitWithXComponent) {
  // Ry(+25) * Rx(+35) applied to (0,0,1)^T = (sin(25)*cos(35), -sin(35), cos(25)*cos(35))^T.
  // Distinguish from the pure Rx presets (Column/Plate/Lowitz) where Z maps to (0, *, *).
  float r[16] = { 0 };
  DefaultPreviewRotation(AxisPreset::kRandom, r);
  ...
  // Sanity: X-component is non-zero (not a pure-X-tilt preset).
