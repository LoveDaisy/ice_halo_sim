...
cam_uv_offset = [0, 0];
cam_proj = @(sph)camera_project(sph, [90, 0, 0], 55, ...
    heatmap_size, 'Equiarea');
% cam_proj = @(sph)camera_project(sph, [72, 25, 0], 52, ...
%     heatmap_size, 'linear');
% cam_proj = @(sph)camera_project(sph, [90, 10, 0], 55, ...
%     heatmap_size, 'linear');
...
%         data(:,1:3) = data(:,1:3) + randn(size(data(:,1:3))) * 0.007;
%         data_norm = sqrt(sum(data(:,1:3).^2, 2));
...
heatmap_spec = heatmap_spec / total_w * 4e3 * 10;
