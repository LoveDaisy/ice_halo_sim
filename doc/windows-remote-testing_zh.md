...
> **只写角色，不写主机名。** 本指南描述的是 **GUI 物理桌面机**这个角色。当前由哪台机器承担、
> 它的仓库与工具链路径是什么，全部在 [`machines.md`](machines.md) —— 主机绑定的单一真源。
> 换机器只改那一份，本指南不必跟着改。
>
> ⚠️ **验证状态：机器更换之后，本流程尚未重新跑过。** 当前承担该角色的机器只做过
> **编译与吞吐**方面的配置与验证，`win_remote_test.sh` / `win_test_watcher.ps1` 未在其上执行过。
> 下面的步骤属于沿用而非新近确认 —— 首次使用时预期需要调试共享目录权限与 watcher 对会话的假设。
