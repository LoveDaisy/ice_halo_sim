...
- **`is_prior_filter_failed_` 是唯一的跨-MS-层持久 per-ray bool**，经 `TraceRayBasicInfo` 前向传播、只在 `InitRayFirstMs` 重置。**"把 per-ray 标志跨层前向携带"的机制已存在、已在三后端续传引擎（scrum-267）跑通**——component 掩码复用它，从 1 bool 加宽成掩码。
...
每层 gate (CollectData):
   ...
   OR 进 per-ray 累积掩码，跨 MS 层前向携带 (加宽 is_prior_filter_failed_，走 TraceRayBasicInfo)
   │
...
| 5 | 掩码怎么跨层 | ✅ 加宽 `is_prior_filter_failed_` 前向传播 | 唯一现存跨层 per-ray 状态，机制已通三后端 |
...
1. **功能1** —— 已完成（task-323）。
2. **功能2 分三阶段，先解耦**：
   - **phase-1 地基（core + CPU）**：component 掩码产出（复用 §5.1）+ 跨层前向累积（加宽 `is_prior_filter_failed_`）+ 交付 consumer。**CPU 白盒**（§4.5）钉死三后端前需验的**跨层掩码一致性语义**——per-component/per-掩码值 出图逐光线核对。这是最大正确性风险，最省成本地先钉死。
...
- **本轮（2026-07-05）核实的代码事实**：filter↔crystal 1:1（`proj_config.hpp`、`filter-architecture.md §1`）；多值 OR 已是 `ComplexFilterParam`（`file_io.cpp::SerializeFilterForCore`）；跨层链 `prev_ray_idx_`/`root_ray_idx_` + `rp_` per-crystal + 唯一跨层 per-ray bool `is_prior_filter_failed_`（`doc/raypath-rayseg-architecture.md`、`core/raypath.hpp`、`simulator.cpp`）。
