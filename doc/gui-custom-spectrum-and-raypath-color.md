...
> 📌 **符号名时效提示（2026-08-10 核对）**：本文是**设计与决策记录**，其中的 as-built 段落记的是
> **写作当时**的形态，按记录性质原样保留。读时请代入两处已发生的更名，别照抄符号：
> - `LUMICE_GetRawXyzResults` / `LUMICE_GetCompositeResults` / `LUMICE_GetRenderResults` /
>   `LUMICE_GetStatsResults` 等六个 server-taking getter **已于 v4.15 净删**，现为
>   `LUMICE_AcquireResultFrame` + `LUMICE_FrameGetRawXyz` / `FrameGetComposite` /
>   `FrameGetRender` / `FrameGetStats`（同一帧内两次读取在构造上同世代，故原先那个
>   "RawXyz+Composite 配对 getter" 与 §4.0 里描述的 host 侧代际漂移 recheck 一并失去了存在理由）。
>   所有权语义见 [`doc/capi-lifecycle-architecture.md`](capi-lifecycle-architecture.md) §9。
> - 文中点名的 `test/gui/functional/test_gui_*.cpp` 系列文件已被 GUI 套件重写取代；
>   下方两处已就地标注新落点，其余请按用例语义搜索，勿按文件名。
...
**⭐as-built（④preview 合成 v1）**：GUI 预览按帧感知 `raypath_color` 激活状态，激活时用 host 侧合成图取代 XYZ 上传，未激活逐字节回落到 XYZ 原路：
...
- **上传分流**（`src/gui/app.cpp` `SyncFromPoller`）：按合成图是否生效分派到 `UploadRadianceTexture`（`TextureMode::kSrgbRadiance`）或 `UploadXyzTexture`（原路，`TextureMode::kXyz`）。**⚠️ 这一条已随"显示端统一补渐晕"的改动改写**：原文写的是"分派到既有 `UploadTexture` + `u_xyz_mode=0`，零 shader 改"，那已不成立——服务端合成图不再把背景色烤进像素，`u_xyz_mode` 也已换成三态 `u_tex_mode`（`kTexModeSrgbComposited` / `kTexModeXyz` / `kTexModeSrgbRadiance`），天空色与镜头相对照度对两条分支一视同仁地由 shader 在显示时加。`UploadTexture`（`kSrgbComposited`，像素里已含天空色、不可分离）今天唯一的生产者是 pre-v4 的旧 `.lmc`。**snapshot_intensity/effective_pixels/texture_upload_count/last_uploaded_texture_serial/p99_raw_y/ev_auto 一字未改**，继续统一来自 xyz_data 派生字段。
...
### 4.8 ⭐合成算法重设计:painter = 亮度即 alpha 的 over 合成(2026-07-15 owner×AI 定案)

> **背景**:scrum-362(task-metal-green-pixel-floor 机制调查)暴露 `dominant` 合成的**噪声敏感**——见 `scratchpad/backlog.md`「渲染/合成算法」条(本节落地后从 backlog 移除)。owner×AI 一次设计 session 把三模式当一个整体重推,收敛出下面的定案。**改 compositor(`src/server/component_compositor.cpp`)前先读本节。**

**根因(机制层,a04):`argmax` 是一个不连续的归约。** `dominant` = 每像素 `argmax_c ey_c`(`component_compositor.cpp:70` `CompositeDominantPixel`)。tie 附近一个无穷小扰动就翻转胜者 → 输出颜色整类跳变(RED→BLUE)。噪声、`ray_num`、跨后端的任何微小差异都被这个不连续性放大成"类归属剧变"。**即使各类 radiance 完全正确也发生**——scrum-362 用 additive 隔离测到跨层 AND 类 MAGENTA 的 radiance Metal/CPU=1.05(1:1),证明 radiance 无问题、抖动纯来自 argmax。这与 362.2 修的"dominant pixel-count 跨后端断言不可移植"是**同一个根、两处症状**(测试断言层面 vs 渲染效果层面)。

**定案 1 —— `dominant` 保持 hard argmax 原样,不连续化。** 曾考虑近 tie 软混合(softmax/幂归一)或置信度去饱和,但 owner 判定二者观感都一般;更重要的是 `dominant` 的抖动是"谁赢"这个**硬分类问题的固有属性**,把它当**诊断视图**(order-free 的硬标签图)是诚实的。软化它不如把好看的默认交给下面的 painter。故 `dominant`/`additive` 都不动。

**定案 2 —— `painter` 重设计为亮度即 alpha 的 over 合成。** 当前 painter(`component_compositor.cpp:111`)是**二值遮挡**:列表序第一个 `ey>0` 的类完全获胜、画 `color·ey`。footgun:一个 `ey=0.001` 的**暗顶点 100% 压住**下面 `ey=100` 的亮点 → 画成近黑(「暗点压亮点看起来黑」)。改为经典 **Porter-Duff "over"**,`alpha` 由亮度驱动:

```
alpha_c = f(ey_c),  f 取线性 clamp:  f(ey) = min(ey, 1)
out = over( alpha_c · color_c )      # 黑底, back-to-front 或 front-to-back 结果相同
```

- **修掉黑洞**:遮挡从二值变成**正比于亮度**——暗顶点 `alpha` 小 → 挡不住 → 亮底点透上来。
- **顺带连续**:over 对 `ey` 连续,无 argmax 不连续 → 重叠区噪声抖动一并解决。
- **能量感知**:亮度仍反映在输出(见下"纯色"要点)。

**⭐要点:over 只作用于纯色相(亮度住在 alpha 槽)。** over 的颜色槽放**色相** `color_c`、亮度全部经 `f()` 进 **alpha 槽**。黑底上单类 = `alpha·color = f(ey)·color`——亮度只进一次。**反例(必须避免)**:颜色槽放 `color·ey` 且 `alpha=f(ey)` → 亮度进两次 = 有效亮度曲线变 `ey²`(单类情形),且重叠时遮挡衰减 `(1−ey)` 与颜色里的 `ey²` **互相打架、语义损坏**(不是单纯变陡)。**关键洞察:"Y 当 alpha"与"Y 当亮度"在黑底上是同一个东西**——无法二选一,因为 `显示=alpha·color`;想让 Y"只管覆盖不管亮度"就得 `f` 硬阶跃,而硬阶跃 `alpha` 一到 1 就恢复二值遮挡 = 正是要修的 bug。所以**必须软斜坡 `f`,而软斜坡在黑底上必然让暗处变暗**。

**唯一旋钮 = `f` 的陡峭度**,此消彼长:平缓(近线性)= 发光曲线 + 保 halo 亮度落差 + 温和透叠;陡峭(近阶跃)= 平涂色块 + 内部均匀 + 但遮挡变硬、黑洞回归。**因为这是 halo 渲染器、亮度渐变是视觉灵魂,默认取平缓的 `f=min(ey,1)`**(P99 以下线性、最亮 1% 饱和到满色),基本无魔数,符合 a05。

**定案 3 —— EV 解耦(painter 特有,关键正确性)。** `dominant`/`additive` 里 display EV 只是亮度乘子(`out=color·ey`);但 painter 里 `alpha=f(ey)`,若 `ey` 含 display EV,**EV 就会同时改不透明度/遮挡**——拉高 EV 整图塌成色块、上层遮得更狠,EV 不再是纯亮度旋钮。故 painter 分支必须拆开两个曝光量:

```
A       = ParticipatingExposureScale(participating_p99)   # 只含 self-anchor, 不含 display EV
alpha_c = min(lane_c · A, 1)                              # 遮挡结构, EV 无关
rgb     = over( alpha_c · color_c )                       # 已归一 ∈ [0,1]
out     = clamp( display_exposure_scale · rgb , 0, 1 )    # display EV = 纯后置亮度乘子
```

- alpha 由 self-anchor 定 → **遮挡/版面结构在拉 EV 时不变**(一条暗弧不会因为你把显示调亮就开始遮住亮弧)。
- self-anchor 反馈完好(task-347 Fix B):藏亮类 → participating 集缩 → P99 降 → `A` 升 → 剩下类 alpha 升、更显。
- 现有代码 `s = ParticipatingExposureScale(p99) · display_exposure_scale`(`component_compositor.cpp:231`)把两者揉在一起;**painter 分支要把 `display_exposure_scale` 从 alpha 路径拎出来、改后置乘**,`dominant`/`additive` 仍走老 `color·ey`(每模式差异值得)。
- 固有性质(非 bug,doc 记明):painter 输出被 `color` 量级封顶(满 alpha 时 = `color` 本身),无 additive/dominant 那种冲过 1 再 clip 的 HDR headroom → EV 拉很高时 painter 更早"到顶"。

**色彩空间约定(核实过 `component_compositor.cpp` + `render.cpp`,a01):**
- **composite(over) 与 display EV 都在线性 RGB**:`CompositeColorClassesLinear` 全程线性,`out_linear_rgb` 是线性值,**sRGB 传递函数只在末端** `LinearRgbToSrgbU8`(`:268`)施加。alpha 合成 = 光的混合,**必须在线性光**否则发暗发浑;EV 是光强乘法,同理。painter over 天然在此阶段——**纪律:别把 over/EV 挪到 `LinearRgbToSrgbU8` 之后**。
- **XYZ-EV 对染色 composite 不适用**:染色 lane 存**标量 Y**(§4.3),色度全来自 `color_`,lane 里无 chroma → EV 就是对 Y 的标量乘,"线性 RGB 乘 Y"与"XYZ 乘 Y"同一操作。XYZ-EV 只对 mono(真光谱)路径有意义,`render.cpp:506` 早已如此(先 `ExposureScale()` 施于 XYZ 再 `XyzToLinearRgb`)。
- **已知简化(不在本轮改)**:类颜色按**线性乘子**消费,但 GUI `ColorEdit3`(`color_window.cpp:890`)按 sRGB 显示空间授权。纯原色(分量全 0/1)无差;仅中间调/低饱和类色会比色块**偏暗**。与 mono `ray_color_` tint(`render.cpp:530` 也当线性)一致,非本设计新引入。要"所见即所得"需 ingest 处 sRGB→linear 且两路径同改,属独立横切决策。

**三模式格局(定案):一个好看默认 + 两个诚实诊断**

| 模式 | 回答的问题 | 性质 |
|------|-----------|------|
| **painter(alpha-over)** | 分层、能量感知的场景 | 连续、能量感知、修了黑洞 → **设为默认** |
