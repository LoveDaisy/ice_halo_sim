...
2. **功能2 分三阶段，先解耦**：
   ...
   - **phase-2 GPU parity**：Metal/CUDA gate 产同一 component 掩码，parity 锚定（对解析/CPU 目标，非旧采样器）。**Metal 已完成**（scrum-331 T5，commit `70ea738c`）；**CUDA 待远程机**。⚠️ device 续传 shuffle（`cont_d` 等）必须一并携带掩码（见 §4.2 陷阱）。
   - **phase-3 上层**：定 consumer binning（§4.3）+ rule editor + 全局 filter 列表面板（§4.4）+ display 多色合成 shader + none-filter 对称色。⚠️ **Metal 侧 handoff**：T5 把"产位"与 test-only host capture ring 共用了 `capture_component` 一个 flag；phase-3 须解耦——产位 → device 侧 component 累加（per-component atomicAdd lanes），不走 host readback ring。
