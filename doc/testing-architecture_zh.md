...
## §7.6 撞线预算红了：该做什么、不该做什么

> 英文版 `testing-architecture.md` 已有完整的 §7（测试范围契约：quick/full/pr 三档各自的实测成本、
> 各档比更便宜那档多抓到什么、预算纪律、独立复核为何是固定倍数、`parity` tag 为何不在 CI）；
> 本文件尚未镜像那整个 §7（属于既有缺口，不由本节引入），这里只同步新增的这一小节，其编号与
> 英文版 §7.6 对齐，供两侧交叉引用。

`e2e-test` job 的 `timeout-minutes: 10`（以及 `e2e-slow` 的 `timeout-minutes: 15`）是 **owner 设定
的预算，不是挂死探测器**。这个区分决定了两种相反的应对：挂死探测器触发意味着什么东西卡住了，
...
两个 job 的 `pytest` 调用都已加上 `--durations=20`，于是处置纪律有了具体的第一步：

1. **撞线红了，先读那次 run 的 `--durations=20` 输出，再做别的事**。它把那次失败的 run 里最慢的
   用例排出来，这是唯一能把"这套测试太慢了"变成"具体是这几个用例"的证据。
2. **不要把重跑当第一反应**。重跑碰巧变绿，说明的不是预算还有余量，只是这次的噪声没有落在已经
   耗尽的那条边界的错误一侧。不看 durations 直接重跑，正是本节要点名并制止的那个失效模式。
3. **从排好序的清单里判断该砍什么、该改什么**，判断标准与 §7.3（英文版）以及 `AGENTS.md` 里
   "完整性主张同样要举证"那条纪律一致：一个慢用例能在这里留下，靠的是它能抓住的缺陷，而不是靠
   它比预算收紧的时间点更早存在。
4. **动预算本身放在最后，而不是最先**。任何一个 job 的 `timeout-minutes` 都由 owner 决定，不是
   实施者可以就地拍板的事——调高它等于把"我们的套件长过了我们愿意为它付的开销"换成"我们多付了"，
   这正是 §7.3（英文版）开篇那段描述的漂移，也正是设预算要拦住的东西。
5. **只有能指名一个与套件规模/内容无关的外因时才重跑**——runner 供给的偶发抖动、一次瞬时的网络
   拉取失败、仓库之外某个不稳定依赖——并把这个外因写下来。一次没有记明外因的重跑，事后无法与
   第 2 条禁止的那种重跑区分开。
