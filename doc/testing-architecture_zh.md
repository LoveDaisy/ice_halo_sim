...
|---|---|---|
| `defaults_panel_layout`（6 景） | 60.56–61.99 dB，地板 40 dB；**5 次重复逐位相同** | 是 |
| `modal_layout`（4 景） | 47.12–48.23 dB，地板 40 dB；**5 次重复逐位相同** | 是 |
...
**这张表没法沉默带过的一条告诫。** 上面每个数字都测自一个 **arm64** Docker 容器（`ubuntu:24.04` +
