...
`RenderConfig` 不持有 filter 字段。task-revert（219.4）之后，先前存在的 `ms_filter_` 字段已删除。
...
**实现方式**：`config_manager.cpp:194-225` 新增了 post-parse 自动绑定，将
`scattering.entries[].filter` 复制到 `renderer.ms_filter_`，激活了 `render.cpp` 中此前休眠的 `FilterRay` 路径。
