...
通过 F1 anchor lane 实现 Off 模式（见 §7）。ABI swap 移除 `unfiltered_*` 字段，引入 `anchor_p995_y` / `anchor_snapshot_intensity`。可加性测试（划分不变式）加入 E2E 测试套件。
...
| 字段 | 语义 |
|------|------|
| `anchor_p995_y` | filter-pass + filter-fail 合并发射的 Y 通道 P99.5（filter-independent EV 锚点）。未配置 filter 时为零（anchor lane 为空）。 |
...
```
filter 存在：  ev = log2(target_linear / (anchor_p995_y / anchor_snapshot_intensity))
                （F1 合并锚；filter-independent）

无 filter：    ev = log2(target_linear / (ComputeP995Y(xyz_buffer) / snapshot_intensity))
                （anchor_p995_y == 0 → 退化；无 filter 时 xyz_buffer 等同全量发射，
...
`anchor_p995_y` 与 `anchor_snapshot_intensity` 暴露给 C API。
...
| C API anchor 字段（Off 模式） | `src/include/lumice.h` — `LUMICE_RawXyzResult.anchor_p995_y` / `anchor_snapshot_intensity` |
