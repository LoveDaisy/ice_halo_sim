...
## §7 自适应亮度 Off 模式——F1 Anchor Lane（scrum-221）

> **历史背景**：scrum-221 之前，task-revert（219.4）恢复 Design A 后，Off 模式曾暂时禁用。
> 本节描述的重设计已在 scrum-221（`feat: F1 mode-toggle + anchor lane`）中落地。

### ABI swap——`unfiltered_*` 字段已移除

task-200 引入的 `unfiltered_xyz_buffer` 和 `unfiltered_snapshot_intensity` 字段已在
scrum-221 ABI swap 中从 `LUMICE_RawXyzResult` 中**移除**。替换字段为：

| 新字段 | 语义 |
|--------|------|
| `anchor_p99_y` | filter-pass + filter-fail 合并发射的 Y 通道 P99（filter-independent EV 锚点）。On 模式或 Off 模式未分配 anchor lane 时为零。 |
| `anchor_snapshot_intensity` | 同一合并集合的 per-pixel 落地强度。On 模式时为零。 |

当前字段列表参见 `src/include/lumice.h` — `LUMICE_RawXyzResult`。

### ε 与 mode-toggle

EV 偏移量（此处宽泛记为 ε；正式定义 `ev = log2(target_linear / p99_norm)`）根据当前自适应亮度模式选择数据源：

```
mode = ON:   ev = log2(target_linear / (ComputeP99Y(xyz_buffer) / snapshot_intensity))
             （Design A 每帧自锚；切换 filter 改变锚点）
...
`src/server/render.cpp` 中的 `RenderConsumer::Consume()` 驱动 anchor 累积。
`PrepareSnapshot()` 将 `anchor_p99_y` 和 `anchor_snapshot_intensity` 暴露给 C API。

无 filter 配置时不分配 anchor lane；Off 模式回退到已过滤路径（此时已过滤路径等同未过滤路径，行为与 On 模式一致，用户无感知）。
...
On 模式使用正常的已过滤 XYZ buffer 作为 P99 锚点（Design A 每帧自锚，§2）。
Off 模式在此之上叠加 anchor lane，不改变 simulation 核心或 filter 门控逻辑。
两种模式共用同一 simulator 二进制；切换模式触发 simulator 重启。

完整模式语义与可加性不变量见 `doc/adaptive-brightness.md`。
