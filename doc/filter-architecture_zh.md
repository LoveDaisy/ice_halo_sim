...
通过 F1 anchor lane 实现 Off 模式（见 §7）。ABI swap 移除 `unfiltered_*` 字段，引入 `anchor_p99_y` / `anchor_snapshot_intensity`（在 apply-new-defaults 中重命名为 `anchor_p995_y`）。可加性测试（划分不变式）加入 E2E 测试套件。
