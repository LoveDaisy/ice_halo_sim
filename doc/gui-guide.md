...
`Angular Distance` holds the two families of iso-angular rings as two rows over the same columns as the main table — swatch, name, `Line`, `Label`, `Alpha`, fold. The name column says what each row is measured **from**; the header already says what they are. `From Sun` is the classic halo rings centred on the sun, 22° / 46° by default (persisted as `grid.angular_dist` / `angular_dist_line` / `angular_dist_label`). `From Axis` is the same kind of ring centred on the camera's **optical axis** — the view centre — so it stays put on the canvas while the view is turned and the sun rings slide past it (`grid.view_dist` / `view_dist_line` / `view_dist_label`, shaped exactly like the `angular_dist` three). Each row's `⋯` fold opens its own angle editor — the same preset-plus-custom editor for both (presets 9° / 22° / 28° / 46°, the halo radii; an axis ring's useful radius depends on the lens, so type it as a custom angle), each editing only its own list, and offered whether or not that row's rings are currently drawn. The `From Axis` list starts **empty**, unlike `From Sun`'s 22° / 46°: ticking its `Line` on a new document draws nothing until an angle is added. The section header carries no buttons of its own — no `⋯`, and none of the `All` / `None` pair `Reference Points` has — because nothing here is family-wide: two line switches, two label switches and two alphas are all per row. Reaching either row's switches or editor therefore starts with unfolding the section.
