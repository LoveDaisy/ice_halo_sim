...
**A hand-written config can still ask for a wildcard.** The dropped-blank-row rule above is about a row that carries no predicate at all. A core config file (the CLI format, or one this GUI exported) can write a raypath filter with an explicitly empty face list — `{"type": "raypath", "raypath": []}` — and that *is* a match-all wildcard, deliberately stated, so it is kept as one and can sit beside real alternatives in a composition. The two are different things arriving by different routes: an empty editor row is an unfinished thought, an empty `raypath` array is a request.
