...
Under the table sit two independent sections, each a collapsing header that is closed by default and does not add a row to the table: `View Circles`, then `Reference Points` (the generalisation of the zenith/nadir markers to a family of six named sky points, one click away rather than six more rows). Their open/closed state is saved with the document.

`View Circles` is the fifth line family: iso-angular rings centred on the camera's **optical axis** — the view centre — rather than on the sun, so a ring stays put on the canvas while the view is turned and `Angular Dist.` rings slide past it. The persisted keys are `grid.view_dist` / `view_dist_line` / `view_dist_label`, shaped exactly like the `angular_dist` three. Opened, the section shows a single row over the same columns as the main table: the colour swatch, `From Axis` (the name column says what the row is measured from — the header already says what it is), `Line` and `Label` checkboxes, and the `Alpha` cell. Its angle editor hangs off the section header's own `⋯` button — the same preset-plus-custom editor `Angular Dist.` opens behind its row fold (so the presets it offers are the halo radii, 9° / 22° / 28° / 46°; an axis ring's useful radius depends on the lens, so type it as a custom angle). The list starts **empty**, unlike the sun circles' 22° / 46°: ticking `Line` on a new document draws nothing until an angle is added. The row's own fold cell stays empty, because a second button to the same editor would be one too many. There are no `All` / `None` buttons on this header, unlike `Reference Points`: one line switch and one label switch leave nothing to batch.
