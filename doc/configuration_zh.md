...
| 字段 | 类型 | 必填 | 默认值 | 说明 |
|------|------|------|--------|------|
| `central` | 对象数组 | 否 | [] | **解析但不渲染。** 这些线会被读取、校验并原样写回，但没有任何代码绘制它们。见下方「不被绘制的网格线」。 |
| `elevation` | 对象数组 | 否 | [] | **解析但不渲染。** 同 `central`。 |
| `outline` | 布尔值 | 否 | false | 沿天球地平线（仰角 0）画一条线，只画在可见半球内。默认关闭，需显式设为 `true` 才绘制。 |

**不被绘制的网格线**

`central` 与 `elevation` 描述逐条线的外观（`value` / `width` / `opacity` / `color`），而渲染器对
这两个列表没有任何消费者——设置了它们的配置能正常解析、比较与序列化，产出的图像与不设置时完全一致。
之所以保留在 schema 里而不是删掉：GUI 预览有自己的一套仰角/方位网格线与太阳角距圈，而两边的描述形态
并不相同（GUI 由 FOV 推出单一步长、共用一种颜色，本 schema 则逐条命名每一根线）。把两者对齐是一个
设计问题而不是实现缺口，所以键保留，并由这段说明如实写明它们今天的行为。`outline` 是这个对象里唯一
真正会绘制的成员。
