...
| `elevation` | 对象数组 | 否 | [] | **解析但不渲染。** 这些线会被读取、校验并原样写回，但没有任何代码绘制它们。见下方「不被绘制的网格线」。 |
...
**`angular_dist` 画什么、忽略什么**

`value`、`opacity`、`color` 全部生效：每条线各自成圈、各自上色、各自按自己的不透明度混合。
`width` 会被读取、校验并原样写回，但**不影响任何像素**——线的粗细来自角距场的局部梯度，这样一条
曲线穿过投影被拉伸的区域时仍保持一条线的宽度，而这个算法没有任何宽度入口。设置了 `width` 的配置
不算错误，只是画出来一样。

**不被绘制的网格线**

`elevation` 描述逐条线的外观（`value` / `width` / `opacity` / `color`），而渲染器对这个列表没有
任何消费者——设置了它的配置能正常解析、比较与序列化，产出的图像与不设置时完全一致。之所以保留在
schema 里而不是删掉：GUI 预览有自己的一套仰角/方位网格线，而两边的描述形态并不相同（GUI 由 FOV
推出单一步长、共用一种颜色，本 schema 则逐条命名每一根线）。把两者对齐是一个设计问题而不是实现
缺口，所以键保留，并由这段说明如实写明它今天的行为。

`angular_dist` 在有渲染器之前也在这份名单上；它并不需要上面那种对齐，因为 GUI 的太阳角距圈本来
就是一份显式的角度列表——只有外观是共用的，而本 schema 用「逐条重复同一份外观」就能表达。
