...
The analysis also always deals rays across crystals by their population share, whatever the **Adaptive ray allocation** setting in Settings says — that switch shapes the *render's* noise, and an analysis is a count of energy per raypath rather than a picture, so its totals are the same either way. Only the render preview changes with the setting.
