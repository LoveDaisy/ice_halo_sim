...
> **待裁决（2026-08-06，非补丁，等 owner 拍板）：I3/I4 两条回归背后浮出一个候选第七条不变量。**
>
> 完成契约的排空语义早就写在本文档里——**本文 §5** 已定义 `Completed` =「有限 run 跑完**并 drain
> 干净**（含 zero-output 完成）」——但这句话此前从未被抬进本节「可固化成门禁/测试」的清单，是一次
> **转录损失**：语义在设计里存在了数月，却不在任何人核对合规时会看的地方。两个已知的历史缺陷
> （终帧被质量闸吞掉的卡死、消费端排空前终态被观测到导致部分总和）都可以读成同一处缺失清单条目的
> 两次代价。
>
> 候选表述：
>
> **I7 完成蕴含排空**：任何向消费方宣告终态的谓词（`SimLifecycle::Completed` / `kIdle`），必须
> 蕴含**该世代的数据已被消费端完全排空**——即终态宣告之后读到的累加量即为终值；且终态之后的销毁
> 路径不得丢弃未消费批次。
>
> 机制现已落地，与措辞是否升格为 I7 无关：`LUMICE_GetDrainStatus(server, &out)` 返回
> `{drained_epoch, current_epoch}`，契约「相等 ⟺ 该世代已排空」，由消费线程在排空时发布（见
> `doc/capi-lifecycle-architecture.md`）；`ShouldSelfPause` 已把它消费为自暂停 guard 的三个条件
> 之一（见上方 I3 补丁）。
>
> **待 owner 决定**：
> 1. 这条契约是否正式升格为 I7、措辞是否即用候选表述；
> 2. 若升格，本文 §5 的 `Completed` 定义是否需要同步改写，使其显式指向 I7——而不是像现在这样，
>    排空语义写在 §5、却要靠读者自己发现它该出现在 §9。
>
> 处置纪律：本节只把决策所需的位置和素材准备好，不代 owner 拍板；不因待裁决而阻塞本文档其余内容。
