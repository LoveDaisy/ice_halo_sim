...
> 与 tag/release `v4.4.2-new`。完整记录见 §8。
>
...
**事实。** v4.4.2（老 shell）与 v4.4.2-new（本文 §1–§4 的形态 + `doc/gui-visual-language.md` 的视觉语言）
交给小范围内测用户对比使用，几乎所有人选择回到老 shell。`main` 随即回退到 PR #271：视觉语言层留下，
形态层（PR #272/#273）退出，实现保存在分支 `feat/new-gui-layout`，永不丢失。
