...
> ⛔ 不得据本文重启面板重排——重启的前提是先有新证据说明内测拒绝的到底是什么，见 §8。
>
...
**这份反馈的粒度是未知的，别当成已知。** 内测结论是聚合的，没有区分被拒的是**形态**（本文）还是**外观**
（`gui-visual-language.md`）。保留视觉语言层的决定既不由这份反馈支持、也不被它否定——它只是把两层分开，
好让下一次取证能问到点上：在老 shell 上单独发一版只带视觉语言层的构建，交同一批内测用户做窄 A/B。
在拿到那个结果之前，任何「他们其实是嫌 X」的说法都是猜测。

**方法层的教训比形态层的结论更值钱。** §1–§4 的形态过了两道验收：可交互 HTML 原型逐项验证，
以及 owner 上手定案。真实用户内测给出了相反结论。这两道验收共享同一个盲区——它们都在问
「这个形态本身好不好」，都没有问「一个已经熟练使用老界面的人是否愿意换」。内测问的正是后者。
⇒ 下次重排任何**既有**界面之前，验收链里必须有一条能测到迁移成本的证据，而不是只测新形态自身的优劣。
