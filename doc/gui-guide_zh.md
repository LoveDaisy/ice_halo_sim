...
`Angular Distance from...` 把两族等角距环放成两行，列与主表相同：颜色块、名称、`Line`、`Label`、`Alpha`、折叠。表头已经说了这一行是**相对什么**度量的；名称列只说是谁。`Sun` 是以太阳为圆心的经典晕环，默认 22° / 46°（持久化为 `grid.angular_dist` / `angular_dist_line` / `angular_dist_label`）。`Lens Center` 是同类的环，但以相机**光轴**（即视野中心）为圆心，所以转动视角时它固定在画布上不动，而太阳环会从它旁边滑过（`grid.view_dist` / `view_dist_line` / `view_dist_label`，与 `angular_dist` 三键完全同形）。每一行自己的 `⋯` 折叠打开自己的角度编辑器——两行用的是同一个"预设 + 自定义"编辑器（预设为晕环半径 9° / 22° / 28° / 46°；光轴环有用的半径取决于镜头，按自定义角度键入即可），各自只编辑各自的列表，且无论该行的环当前画不画都提供。`Lens Center` 的列表初始为**空**，不像 `Sun` 默认带 22° / 46°：新文档里只勾上它的 `Line` 什么都不会画，要先加一个角度。分段表头自己不带任何按钮——没有 `⋯`，也没有 `Reference Points` 那样的 `All` / `None`——因为这里没有任何"全族共用"的东西：两个线开关、两个标签开关、两个 alpha 全是逐行的。所以要触达任一行的开关或编辑器，第一步都是展开这个分段。
