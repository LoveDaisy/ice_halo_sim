...
//
// `fwidth` is |dF/dx| + |dF/dy| in degrees per pixel, so a fixed count of PIXELS (the multiplier)
// becomes a locally correct count of degrees, and the line reads the same width at a 1 degree field
// of view and at 180. The CLI evaluates that rule on the CPU over a rendered canvas
// (src/core/lens_proj_build.hpp, mask_detail::LevelSetMaskFromField, with a forward difference in
// place of the rasterizer's derivative); the GUI preview evaluates it per fragment in its shader
// (src/gui/preview_renderer.cpp, overlayAuxLines, with the hardware fwidth()). Two evaluators of ONE
// definition, on the hardware each one has — and the three numbers below are that definition's
// only free parameters, so this is where they live, once.
//
...
// Lower clamp on the local gradient, in degrees per pixel. Not zero, for two reasons that both
// bite: a zero half-width makes the shader's smoothstep(0, 0, x) undefined (edge0 == edge1), and on
// the CPU a pixel whose two measured neighbours read the same value would collapse the line to
// nothing. Low enough that it never binds at any field of view this renderer supports — the
// narrowest is 1 degree over hundreds of pixels, ~1e-3 deg/px — so fwidth is honoured wherever it
// is finite. (A higher floor pins the degrees-per-pixel conversion and makes the line grow with
// magnification; that was a measured defect, not a hypothetical.)
