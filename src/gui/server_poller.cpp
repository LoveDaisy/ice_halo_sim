...
void ServerPoller::Start(LUMICE_Server* server) {
  ...
  // This is outside data_mutex_ because only the Start() caller writes it before
...
void ServerPoller::WorkerLoop(LUMICE_Server* server) {
  while (running_) {
    ...
    // Short sleep when no valid data yet (catch first texture quickly), normal interval otherwise.
    // Sleep in 1ms increments so Stop() can join quickly.
