...
// GUI-side log pattern (matches Core pattern for consistency)
constexpr const char* kGuiLogPattern = "%Y-%m-%d %H:%M:%S.%e [%L] %v";
...
// GUI-side log macros (mirror Core's LOG_* naming for convenience)
#define GUI_LOG_VERBOSE(...) SPDLOG_LOGGER_TRACE(&lumice::gui::GetGuiLogger(), __VA_ARGS__)
#define GUI_LOG_DEBUG(...) SPDLOG_LOGGER_DEBUG(&lumice::gui::GetGuiLogger(), __VA_ARGS__)
#define GUI_LOG_INFO(...) SPDLOG_LOGGER_INFO(&lumice::gui::GetGuiLogger(), __VA_ARGS__)
#define GUI_LOG_WARNING(...) SPDLOG_LOGGER_WARN(&lumice::gui::GetGuiLogger(), __VA_ARGS__)
#define GUI_LOG_ERROR(...) SPDLOG_LOGGER_ERROR(&lumice::gui::GetGuiLogger(), __VA_ARGS__)
